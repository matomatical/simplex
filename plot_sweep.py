"""Plot factored vs joint probe R² by epsilon from sweep results (mean over seeds)."""
...
def main(results_file: str):
    ...
    # check if R² data is available
    has_r2 = "factored_r2" in results[0]
    ...
    if has_r2:
        ...
        for eps in epsilons:
            runs = by_eps[eps]
            mean_factored.append(np.mean([r["factored_r2"] for r in runs]))
            mean_joint.append(np.mean([r["joint_r2"] for r in runs]))
        ...
        chart = mp.border(chart, title=" factored vs joint probe R² by epsilon (mean) ")
        print(chart)
        ...
        print(f"  {'epsilon':>10s}  {'fac R²':>10s}  {'± se':>8s}  "
              f"{'jnt R²':>10s}  {'± se':>8s}  {'p-value':>10s}  {'sig?':>5s}  "
              f"{'n':>3s}")
        for eps in epsilons:
            runs = by_eps[eps]
            f_vals = np.array([r["factored_r2"] for r in runs])
            j_vals = np.array([r["joint_r2"] for r in runs])
            n = len(runs)
            f_mean = f_vals.mean()
            j_mean = j_vals.mean()
...
if __name__ == "__main__":
    main(sys.argv[1] if len(sys.argv) > 1 else "sweep_results_r2.jsonl")
